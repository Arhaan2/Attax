...
class Board {
    ...
    boolean canMove(PieceColor who) {
        for (char c = 'a'; c < 'h'; c++) {
            for (char r = '1'; r < '8'; r++) {
                if (get(c, r).equals(who)) {
                    for (int dc = -2; dc < 3; dc++) {
                        for (int dr = -2; dr < 3; dr++) {
                            int neigh = neighbor(index(c, r), dc, dr);
                            if (_board[neigh].equals(EMPTY)) {
                                return true;
                            }
                        }
                    }
    ...
    void makeMove(Move move) {
        ...
        for (int x = -1; x < 2; x++) {
            for (int y = -1; y < 2; y++) {
                int neighbour = neighbor(move.toIndex(), x, y);
                if (get(neighbour) == _whoseMove.opposite()) {
                    set(neighbour, _whoseMove);
                    incrPieces(_whoseMove, 1);
    ...
    void undo() {
        while (_undoPieces.peek() != null) {
            int pop = _undoSquares.pop();
            PieceColor currentColumn = _undoPieces.pop();
            if (currentColumn != EMPTY) {
                incrPieces(currentColumn, 1);
            }
            if (get(pop) != EMPTY) {
                incrPieces(get(pop), -1);
            }
            unrecordedSet(pop, currentColumn);
        }
    ...
    boolean legalBlock(char c, char r) {
        ...
        if (get(c, r) != EMPTY || "" + c + r == "g1" || "" + c + r == "a1"
                || "" + c + r == "g7" || "" + c + r == "a7") {
            return false;
        }
        return true;
    ...
    /**
     * Return true iff it is legal
     * to place a block at CR.
     */
    ...
    void setBlock(char c, char r) {
        ...
        char reflectedRow = (char) ('1' + '7' - r);
        char reflectedColumn = (char) ('a' + 'g' - c);

        set(reflectedColumn, reflectedRow, BLOCKED);
        set(c, reflectedRow, BLOCKED);
        set(reflectedColumn, r, BLOCKED);
        set(c, r, BLOCKED);
